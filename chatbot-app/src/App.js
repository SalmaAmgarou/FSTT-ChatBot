...
import './index.css';
import { FiAlignJustify, FiEdit, FiSlack } from "react-icons/fi";
import { FcDecision, FcDribbble, FcLink, FcBinoculars, FcLightAtTheEndOfTunnel } from "react-icons/fc";
...
function App() {
    ...
    const handleSuggestionClick = async (suggestion) => {
        try {
            ...
            // Assuming the response contains the formatted response, extract the answer
            const responseText = res.data.response;
            const answerMatch = responseText.match(/✅Réponse :(.+)/);
            const answerText = answerMatch ? answerMatch[1].trim() : responseText;

            setMessages((prevMessages) => [...prevMessages, { sender: 'bot', text: answerText }]);
        } catch (error) {
            console.error('Error querying model:', error);
        } finally {
            setSuggestions([]); // Remove all suggestions
    ...
    return (
        <div className="flex min-h-screen bg-gray-900 text-white">
            ...
            <button 
                ...
                className="bg-indigo-900 text-white p-2 rounded-full shadow-md focus:outline-none mb-4"
            >
                <FiAlignJustify />
            </button>
                    ...
                    <div className="w-full max-w-lg flex center space-x-4 mb-4">
                        {suggestions.map((suggestion, index) => (
                            <div 
                                ...
                                className="p-4 top-10 left-5 rounded-lg shadow-md bg-gray-700 text-white cursor-pointer w-full text-center relative"
